...
/*
	オプショナルプロパティの型
	オプショナルプロパティの場合、プロパティ自体を省略できる。undefinedとのユニオン型で定義した場合、プロパティの省略はコンパイルエラーになる。
*/
type Human = {
	name: string;
...
## 非同期処理
### 非同期処理とは
TypeScriptでは通信やファイルの読み書きなど時間がかかる処理は基本的に非同期処理（ノンブロッキング）
JavaScriptの実行モデルはシングルスレッドモデルが採用されているため、JavaScriptプログラムの複数箇所が同時に（並列に）実行されることはない。
通信などは厳密にはOSの処理であり、JavaScriptはOSに処理を委任して待ってるだけなので、シングルスレッドでも並列で実行できる。
### コールバックによる非同期処理の扱い
