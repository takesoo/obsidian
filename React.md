...
---
## what
- 設計思想：コンポーネントの組み合わせによるUI開発ライブラリ
- JavaScriptでマークアップする。[[JSX]]
- ルーティングやデータフェッチなどの機能は[[Next.js]]などのフレームワークを使用する。
### Reactにおけるサーバーサイドとクライアントサイド
- コンポーネントをサーバーとクライアントに分割する設計思想
- [[クライアントコンポーネント|Client Components]]: ブラウザで動くコンポーネント
- [[サーバーコンポーネント|React Server Component]]: サーバーで動く
- [[Server Function]] / [[Server Action]]: クライアントからサーバー処理を呼び出す仕組み
- サーバーサイド実行環境には[[Next.js]]などのフルスタックwebフレームワークをしy方する。
## how
[[hooks]]
[[Custom Hook]]
