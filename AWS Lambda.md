...
### バージョン
- Lambdaのコードと環境変数はバージョン管理されている
- 修飾ARNで指定可能
	```
	lambda_function_name:$LATEST ← 編集中
	lambda_function_name:3 ← v3
	```
### Lambdaエイリアス
- 特定の関数バージョンへの名前付きポインタ
- prodやdevなど
- 重み付けエイリアス
	- prodの90%をv5, 10%をv6に振り分けて[[カナリアリリース]]できる
### プロビジョニングされた同時実行(Provisioned Concurrency)
- 同時実行数を事前にプロビジョニングできる。設定した数のコンテナは常にウォームスタートになるため、コールドスタートをある程度防ぐことができる。特にVPC接続されたLambdaの起動で効果的。
- ただし設定数以上のリクエストが来てスケールアウトする場合はコールドスタートになることに注意。
- また、コストが別でかかる点にも注意
### イベントソースマッピング
- ストリームやキューからのデータをLambdaがポーリングする設定
- バッチ処理が可能
- [[DynamoDB]]
- [[AWS Kinesis]]
- [[Amazon MSK]]
...
- Lambdaで利用するデータベース
	- RDBMSはアンチパターン
		- Lambdaはステートレスなプラットフォームなのでなので、コネクションプールの実装が難しい、加えて、スケールしたときに最大同時接続数を超えてしまう
		- コールドスタートした時に時間がかかる
		- 実行数が少なく、コールドスタートによる遅延が許容できるならOK
	- [[DynamoDB]]を使う
	- RDBMSへの反映は非同期にする
		- DynamoDB Streams
		- Kinesis
...
- IP固定はアンチパターン
	- 署名や証明書などで担保する（推奨）
	- VPCを利用してNATGateway使う
- アプリケーションの管理
	- ユニットテストはローカルで実施する
		- ビジネスロジックをハンドラと分けるとテストしやすい
	- インテグレーションテストや受入テストは実際のサービスで
	- CI/CDパイプライン
		- build: CodeBuild
		- beta: CloudFormation
		- test: 3rd party
