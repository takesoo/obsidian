...
## エラーハンドリング
### クライアントサイド
- 入力エラーやAPI失敗は例外にせず、フォームやトーストなどでUIに表示する
- レンダリング中の例外は`error.tsx`で受ける
- イベントハンドラと非同期コールバックでは例外を捕捉してStateでエラー状態を保存して表示する
- `global-error.tsx`は最後の安全網
- 利用者には「何が失敗したか」「入力内容が保持されているか」「次に何をすればよいか」を示し、詳細な例外やスタック トレースは監視サービスへ送る。
- 
---
コンポーネント
[[hooks]]
